...
/// Synchronous variant for "blocking" dependencies.
/// Generates fields, getters, overrides and trait impls that operate synchronously,
/// and use `std::sync::RwLock` instead of `tokio::sync::RwLock`.
pub fn gen_blocking_dep_artifacts(
    ctx_name: &Ident,
    dep_tys: &[Type],
) -> (
    Vec<TokenStream2>, // field defs
...
) // `impl CtxHas…Blocking for Ctx`
{
    ...
    for trait_ty in dep_tys {
        ...
        // helpers that already existed
        let default_fn_ident = format_ident!("__default_{}_blocking", field_ident);
        let ctxhas_ident = format_ident!("CtxHas{}Blocking", chain_to_pascal(&chain));
        let prefix = type_path_prefix(trait_ty);
        ...
        field_defs.push(quote! {
            #[doc(hidden)]
            pub #field_ident: ::std::sync::RwLock<Option<#field_ty_tokens>>
        });

        // 2.  Field init (always `None`)
        field_inits.push(quote! {
            #field_ident: ::std::sync::RwLock::new(None)
        });

        // 3.  Getter + override on the context itself
        let getter_ret_ty = field_ty_tokens.clone(); // same as field type
        getters.push(quote! {
            pub fn #getter_ident(&self) -> ::std::result::Result<
                #getter_ret_ty,
                ::fractic_server_error::ServerError
            > {
                if let Some(existing) = {
                    let read = self.#field_ident.read().expect("RwLock poisoned");
                    (*read).clone()
                } {
                    return ::std::result::Result::Ok(existing);
                }
                ...
                // store – be race-aware
                let mut write = self.#field_ident.write().expect("RwLock poisoned");
                let arc = if let Some(ref arc) = *write {
                    arc.clone()
                } else {
                    write.replace(built.clone());
            ...
            #[cfg(test)]
            pub fn #override_ident(&self, new_impl: #getter_ret_ty) {
                let mut lock = self.#field_ident.write().expect("RwLock poisoned");
                *lock = Some(new_impl);
            }
        });

        // 4.  Blanket `CtxHas…Blocking` impl
        trait_impls.push(quote! {
            impl #ctxhas_path for #ctx_name {
                fn #getter_ident(&self) -> ::std::result::Result<
                    #getter_ret_ty,
                    ::fractic_server_error::ServerError
                > {
                    self.#getter_ident()
